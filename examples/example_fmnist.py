"""Minimal end-to-end SSplain example on FMNIST.

Run from the repository root:

    python examples/example_fmnist.py --index 0 --sparsity 2.0 --max_iteration 20
...
CKPT = os.path.join(
    _HERE, "ckpt", "FMNIST_batchsize32_lr0.001_weight_decay0.0001_model_best.pth.tar"
)
...
def load_model(ckpt_path=CKPT):
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(
            ...
            "Train one first:  python examples/train.py --dataset fmnist"
        )
    model = LeNet()
...
def main():
    ...
    p.add_argument("--lambda_tv", type=float, default=1e-4)     # FashionMNIST
    p.add_argument("--no_show", action="store_true", help="don't open a window")
    args = p.parse_args()

    model = load_model()
    image, label = get_one_image("fmnist", index=args.index)
    print(f"Explaining FashionMNIST test image #{args.index} (true label {label})")
    ...
    out = os.path.join("results", "fmnist_example.png")
